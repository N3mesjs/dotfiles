...
hl.bind(mainMod .. " + SHIFT + 1",                    hl.dsp.window.move({ monitor = "DP-2" }))
hl.bind(mainMod .. " + SHIFT + 2",                    hl.dsp.window.move({ monitor = "DP-3" }))
...
-- SUPER + [1-9, 0] -> Va direttamente al workspace (lo crea al volo ovunque tu sia)
for i = 1, 10 do
    local key = i % 10
    hl.bind(mainMod .. " + " .. key, hl.dsp.focus({ workspace = tostring(i) }))
end

-- SUPER + SHIFT + [1-9, 0] -> Sposta la finestra attiva al workspace
for i = 1, 10 do
    local key = i % 10
    hl.bind(mainMod .. " + SHIFT + " .. key, hl.dsp.window.move({ workspace = tostring(i) }))
end

-- Focus sui monitor (se hai due monitor, usa un altro tasto invece dei numeri, es. virgola e punto)
--hl.bind(mainMod .. " + comma",  hl.dsp.focus({ monitor = "DP-2" }))
--hl.bind(mainMod .. " + period", hl.dsp.focus({ monitor = "DP-3" }))
